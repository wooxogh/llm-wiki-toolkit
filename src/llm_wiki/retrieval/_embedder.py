...
DEVICE = os.environ.get("WIKI_EMBED_DEVICE", "mps")  # mps | cpu | cuda
...
def _resolve_device(requested: str = None) -> str:
    ...
    `DEVICE` defaults to "mps" because that is the machine this was written on,
    but using it unguarded turns "no Apple Silicon" into a raw torch error for
    every other user of the `ml` extra. So an unavailable accelerator falls back
    to CPU — and says so on stderr. Silently running 10-40x slower than the user
    asked for is the worse failure: they would only see an inexplicably long
    rebuild with no hint of why.
    """
...
def _get():
    ...
    if _model is None:
        ...
        _model = SentenceTransformer(MODEL, device=_resolve_device())
...
def _encode(texts: list[str]) -> np.ndarray:
    ...
    v = _get().encode(texts, normalize_embeddings=True, convert_to_numpy=True,
...
def embed_query_local(text: str) -> np.ndarray:
    ...
    return _encode([_QUERY_INSTRUCTION + text])[0]
