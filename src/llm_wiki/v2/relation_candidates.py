...
def discover(vault: Path | None, seed: Concept, concepts: list[Concept], top_k: int = 10) -> list[Concept]:
    ...
    return [c for _, c in sorted(scored, key=lambda row: (-row[0], row[1].id))[:top_k]]
